...
void FreeCamera::updateFreeCamera(const p6::Context& ctx, const MovementInput& keyboard)
{
    // rotation
    if (ctx.mouse_button_is_pressed(p6::Button::Left))
    {
        rotateLeft(-ctx.mouse_delta().y * ctx.delta_time() * 20.);
        rotateFront(-ctx.mouse_delta().x * ctx.delta_time() * 20.);
    }
    ...
    moveUp((keyboard.up_pressed - keyboard.down_pressed) * ctx.delta_time() * 3.);
}
