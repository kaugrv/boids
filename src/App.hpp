...
struct Application {
    ...
    void initialize()
    {
        ...
        list_images_used.push_back(std::make_shared<p6::Image>(p6::load_image("../assets/textures/building.jpg")));

        // Create lights
        DirectionalLight dir_light{.direction = glm::vec3(1., 1., 0.), .color = glm::vec3(1., 1., 1.), .intensity = 2.};
        DirectionalLight dir_light2{.direction = glm::vec3(-1., 1., 0.), .color = glm::vec3(1., 1., 1.), .intensity = 2.};
        PointLight       point_light{.position = glm::vec3{0., 1., 0.}, .color = glm::vec3(0.9, 0.7, 0.8), .intensity = 0.2};
        ...
        Material car_material(MaterialParameters{.diffuse = glm::vec3(0.5, 0.5, 0.5), .reflexion = glm::vec3(0.5), .glossy = glm::vec3(0.5), .shininess = 2., .alpha = 1.}, list_shaders_used[0], list_images_used[0]);
        ...
        Material box_material(MaterialParameters{.diffuse = glm::vec3(0., 0., 1.), .reflexion = glm::vec3(0.5), .glossy = glm::vec3(0.5), .shininess = 2., .alpha = 0.5}, list_shaders_used[0], list_images_used[1]);

        Object3D box{.m_mesh = list_mesh_used[2], .m_material = box_material};
